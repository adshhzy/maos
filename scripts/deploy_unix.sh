...
step "Starting Sandbox, Simulator, Temporal worker, and Temporal server"
sandbox_args=(
  sandbox_service.py
  --host "$HOST"
  --port "$SANDBOX_PORT"
...
step "Health checks"
if [[ "$SKIP_AGENT_SERVICE" != "1" ]]; then
  wait_http "http://$HOST:$AGENT_SERVICE_PORT/health" "Agent Service"
fi
wait_http "http://$HOST:$SANDBOX_PORT/api/health" "Sandbox"

step "Deployment complete"
echo "Web UI:        http://$HOST:$SANDBOX_PORT/"
echo "Sandbox API:   http://$HOST:$SANDBOX_PORT/api/health"
echo "Simulator API: http://$HOST:$SIMULATOR_PORT"
if [[ "$SKIP_AGENT_SERVICE" != "1" ]]; then
