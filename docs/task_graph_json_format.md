...
## Direct Hermes Agent 节点

如果节点只需要轻量分析、总结、评审门判断，推荐使用 direct Hermes provider。它不会创建 Multica task，不会写 Multica comment，也不会进入 Multica 的默认任务工作流；MAOS 会直接启动本地 `hermes.cmd -z`，然后通过 Temporal durable polling 等待结果。
...
Direct Hermes 节点输出会被归一化为普通节点 payload，常用字段包括：

| 字段 | 含义 |
| --- | --- |
| `agent_backend` | 固定为 `hermes`。 |
| `hermes_job_id` | 本地 Hermes provider job id。 |
| `latest_comment` | Hermes stdout 的摘要，供 `$deps.node_id.latest_comment` 引用。 |
| `structured_output` | 如果 stdout 中包含 JSON，会被解析到这里。 |
| `decision` | 如果 JSON 或文本中包含分支决策，会提升到顶层，供 `edge.when` 使用。 |
